...
public class LCD16x2Emulator extends PApplet {
...
public void serialEvent(Serial myPort) {
  inByte = PApplet.parseChar(myPort.read());
  switch (inByte) {
  case 14: 
    moveCursorLeft();
    break;
...
public void writeLCDChar(char character, int x, int y) {
...
public void textCursor(int x, int y) {
  strokeWeight(2);
  stroke(255);
...
public void keyTyped() {
  if (serialActive) myPort.write(PApplet.parseChar(key));
    else {
  if (key == BACKSPACE) {
    moveCursorLeft();
    displayChars[cursorCol][cursorRow] = 0;
...
public void customize(DropdownList ddl, String[] items ) {
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if (theEvent.isGroup()) {
    ...
    serialRef = PApplet.parseInt(theEvent.getGroup().getValue());
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "LCD16x2Emulator" };
    if (passedArgs != null) {
